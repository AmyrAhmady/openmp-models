...
export default class Scene {
  ...
  async init(rootElement: HTMLDivElement | null) {
    ...
    if (this.sceneData.length) {
      if (
        this.sceneData[0].type === "skin" ||
        this.sceneData[0].type === "object"
      ) {
  ...
  async setupSkinOrObject(modelData: ModelData) {
    ...
    requestAnimationFrame(() => {
      ...
      const timerId = setInterval(() => {
        timerCount++;
        if (timerCount == 3) clearInterval(timerId);
        this.renderer.render(this.scene, this.camera);
      }, 350);
    });
  }
  ...
  CreateObjectMesh(
    ...
    for (const msplitKey in Object[i].geometry.textures) {
      ...
      if (matsByName[msplit] === undefined) {
        ...
        if (texture.name) {
          ...
          if (matsByName[msplit].map !== null) {
            matsByName[msplit].map!.wrapS = matsByName[msplit].map!.wrapT =
      ...
      const appendFace = (faceIndices: number[]) => {
        faceIndices.forEach((vertexIndex) => {
          const vertex = Object[i].geometry.vertices[vertexIndex];
          const texcoord = Object[i].geometry.texcoords?.[vertexIndex];

          positions.push(vertex.x, vertex.y, vertex.z);
          uvs.push(texcoord?.uvx || 0, texcoord ? 1 - (texcoord.uvy - 1) : 0);
    ...
    GeometryMesh.setAttribute("uv", new THREE.Float32BufferAttribute(uvs, 2));
    GeometryMesh.setIndex(indices);
